...
def main():
    ...
    dataloader = DataLoader(ImageDataset("data/"),
    ...
    for epoch in range(opt.epoch, opt.n_epochs):
        for i, imgs in enumerate(dataloader):
            ...
            imgs_seg = Variable(input_seg.copy_(imgs['seg']))
            name = imgs['name']
           
            #optimizers
            LR =  opt.lr*(.5 ** (count // 10000))
            count = count + 1
            optimizer_G = torch.optim.Adam(generator.parameters(), lr=LR, betas=(opt.b1, opt.b2))
            optimizer_D = torch.optim.Adam(discriminator.parameters(), lr=LR*.1, betas=(opt.b1, opt.b2))
            ...
            if count >= 1000:
                ...
                # Total loss
                loss_G = loss_gan*5e-3 + loss_content_high*.01
            ...
            print("[Epoch %d/%d] [Batch %d/%d] [D loss: %f] [G loss: %f] [MSE loss: %f] [Learning rate: %f] " %
                                                                (epoch, opt.n_epochs, i, len(dataloader),
                                                                loss_D.item(),
                                                 
                                                                loss_G.item(),loss_mse.item()
                                                                ,LR
